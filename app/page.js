...
export default function Home() {
  ...
  const selectMessage = (id) => {
    const updatedMessages = messages.map((message) =>
      message.id === id ? { ...message, isSelected: true } : { ...message, isSelected: false }
    );
    ...
    <>
      <nav className="flex items-center justify-between px-5 py-5 sm:px-10 font-plus-jakarta-sans font-500 text-16px">
        <div className="flex items-center ">
          ...
          <span className=" font-800  rounded-[4px] bg-Blue text-White text-xs w-6 h-5 flex items-center justify-center">{unreadMessages}</span>
        </div>
        <span
          className="cursor-pointer text-[0.9rem] sm:text-[1rem] text-dark-grayish-blue hover:text-[#0a317b]"
      ...
      {messages.map((message) => (
        <div className=" mx-5 sm:mx-10 grid gap-3 font-plus-jakarta-sans ">
          <div key={message.id}
            className={`flex py-2 px-5 ${message.isRead ? 'bg-White' : 'bg-very-light-grayish-blue'
            ...
            <div className="m-4 flex-grow ">
              <div>
                ...
                <span className="hover:text-[#0a317b] font-800 text-dark-grayish-blue text-[0.95rem] sm:text-[1rem] inline-flex items-center gap-1 cursor-pointer">My first tournament today!
                  <span key={message.id} className={`${message.isRead ? 'bg-none' : 'bg-Red'} sm:[w-1.5 h-1.5] w-2 h-2 rounded-full`}></span>
                </span>
              </div>
